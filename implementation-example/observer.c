/**
  *
  * `brief`     :  Example code that format frames following to the SensorLab2 specifications
...
owerror_t observer_new_event(void){
   ...
   if (observer_vars.cursor != 0 || observer_vars.properties_left_count != 0){
      ...
      /** TODO: beginning of the section of code that should be protected against interruptions **/
      /** DISABLE_INTERRUPTS(); **/
      memset(observer_vars.buffer, 0, OBSERVER_BUFFER_LENGTH * sizeof(uint8_t));
...
owerror_t observer_submit(void){
   ...
   if(observer_vars.cursor == 0){
      ...
      if(!observer_vars.overflow_error){
         /** TODO: call to the serial write function **/
         /** serial_write(observer_vars.buffer, observer_vars.cursor); **/
         observer_vars.cursor = 0;
         observer_vars.properties_left_count = 0;
         /** TODO: end of the section of code that should be protected against interruptions **/
         /** ENABLE_INTERRUPTS(); **/
         return E_SUCCESS;
      }else{
         observer_vars.buffer[0] = OBSERVER_BUFFER_OVERFLOW;
         /** TODO: call to the serial write function **/
         /** serial_write(observer_vars.buffer, 1); **/
         observer_vars.cursor = 0;
         observer_vars.properties_left_count = 0;
         observer_vars.overflow_error = FALSE;
         /** TODO: end of the section of code that should be protected against interruptions **/
